...
from qa.models import (QAAnswer as Answer, AnswerComment, AnswerVote, QAQuestion as Question,
                       QuestionComment, QuestionVote)

from user_profile.models import Profile as Profile
...
class QuestionIndexView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        noans = Question.objects.order_by('-pub_date').filter(
            answer_set__isnull=True).select_related('user')\
            .annotate(num_answers=Count('answer_set', distinct=True),
...
class QuestionsByTagView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['anscount'] = Answer.objects.count
        context['noans'] = Question.objects.order_by('-pub_date').filter(
            tags__name__contains=self.kwargs['tag'], answer_set__isnull=True)[:10]
        context['reward'] = Question.objects.order_by('-reward').filter(
    ...
    def post(self, request, object_id):
        ...
        if vote_target.user == request.user:
            ...
            vote, created = self.vote_model.objects.get_or_create(
                ...
                **object_kwargs)
            if created:
                vote_target.user.profile.reputation += 1 if upvote else -1
                if upvote:
                ...
                if vote.value == upvote:
                    ...
                    vote_target.user.profile.reputation += 2 if upvote else -2
                    vote.value = upvote
                    vote.save()
                    if upvote:
                        vote_target.positive_votes += 1
